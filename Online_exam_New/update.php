...
if (isset($_SESSION['key'])) {
    if (@$_GET['fdid'] && $_SESSION['key'] == 'rama01738039685') {
        $id = @$_GET['fdid'];
...
//delete user
if (isset($_SESSION['key'])) {
    if (@$_GET['demail'] && $_SESSION['key'] == 'rama01738039685') {
        $demail = @$_GET['demail'];
...
//remove quiz
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'rmquiz' && $_SESSION['key'] == 'rama01738039685') {
        $eid = @$_GET['eid'];
...
//add quiz
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == 'rama01738039685') {
        $name = $_POST['name'];
...
//add question
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == 'rama01738039685') {
        $n = @$_GET['n'];
        $eid = @$_GET['eid'];
        $ch = @$_GET['ch'];
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
    ...
    if ($ans == $ansid) {
        ...
        $qp = mysqli_query($con, "INSERT INTO user_answer VALUES('$email','$eid' ,'$qid','$ans' )") or die('Error137');
    ...
    if ($sn != $total) {
    ...
    } else if ($_SESSION['key'] != 'rama01738039685') {
        ...
        // change do something
        $question_count = mysqli_query($con, "SELECT eid FROM questions WHERE eid = '$eid'") or die('Error161');
        $qcount = mysqli_num_rows($question_count);

        $row_query = mysqli_query($con, "SELECT sahi FROM history WHERE eid = '$eid' AND email='$email'") or die('Error161');;

        while ($rows = mysqli_fetch_array($row_query)) {
            $right_answer = $rows['sahi'];
        }
...
if (@$_GET['q'] == 'quizre' && @$_GET['step'] == 25) {
    ...
    $q = mysqli_query($con, "DELETE FROM user_answer WHERE eid='$eid' AND email='$email' ") or die('Error184');
    $q = mysqli_query($con, "SELECT * FROM rank WHERE email='$email'") or die('Error161');
    while ($row = mysqli_fetch_array($q)) {
        $sun = $row['score'];
    }
